...
export default function createApolloServer(options = {}) {
  ...
  const gqlMiddleware = expressMiddleware.filter(
    (def) => (def.route || "").trim() !== "" && !def.methods
  );
  ...
  app.use.apply(app, [
    ...
    ...gqlMiddleware
      ...
      .map((def) => def.fn(contextFromOptions)),
    ...gqlMiddleware
      .filter((def) => def.stage === "api")
      .map((def) => def.fn(contextFromOptions)),
    ...gqlMiddleware
      .filter((def) => def.stage === "webhook")
      .map((def) => def.fn(contextFromOptions)),
  ]);
  //° Methodes
  expressMiddleware.forEach((rq) => {
    if ((rq.route || "").trim() !== "" && rq.methods) {
      rq.methods.split(",").forEach((mth) => {
        console.info(
          `\n»» mth`,
          rq.route,
          mth,
        ...
        app[mth](`/${rq.route}`, rq.fn(contextFromOptions));
      });
    }
  });
  //! by method
